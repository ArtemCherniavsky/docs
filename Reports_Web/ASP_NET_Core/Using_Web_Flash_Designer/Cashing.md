...
**None** – caching is disabled, when printing and exporting, the report will be every time sent to the server-side;
...
{
StiNetCoreDesignerFx.CacheHelper = new StiMyCacheHelper();
}
}
...
```

To initialize the work with report caching using the created class, it is enough to set it as the value of the **StiNetCoreDesignerFx.CacheHelper** static property in the ASPX page constructor.
